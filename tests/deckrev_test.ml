...
and only_green
: type a. a m -> (a, a, is_green, only, nh, nh, nh, not_rev) triple m
= fun e ->
  Nest
  (fun () ->
...
and only_red
: type a. a m -> (a, a, is_red, only, nh, nh, nh, not_rev) triple m
= fun e ->
  Nest
  (fun () ->
    buffer_ge5 e >>= fun p ->
...
and left_green
: type a. a m -> (a, a, is_green, left, nh, nh, nh, not_rev) triple m
= fun e ->
  Nest
  (fun () ->
  ( buffer_ge5 e >>= fun p ->
...
and left_red
: type a. a m -> (a, a, is_red, left, nh, nh, nh, not_rev) triple m
= fun e ->
  Nest
  (fun () ->
    buffer_ge5 e >>= fun p ->
...
and right_green
: type a. a m -> (a, a, is_green, right, nh, nh, nh, not_rev) triple m
= fun e ->
  Nest
  (fun () ->
  ( buffer_eq2 e >>= fun p ->
...
and right_red
: type a. a m -> (a, a, is_red, right, nh, nh, nh, not_rev) triple m
= fun e ->
  Nest
  (fun () ->
    buffer_eq2 e >>= fun p ->
...
let fold_left_to_list t =
  List.rev (Deque.Deckrev.fold_left (fun xs x -> x::xs) [] (Obj.magic t))

let test _ deq =
  let real_deq = D.regular_of_semi deq in
  let lst = to_list real_deq in
  assert (lst = Deque.Deckrev.to_list (Obj.magic real_deq)) ;
